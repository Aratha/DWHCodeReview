...
/** Yorum bloğu kapanışı ile SQL gövdesi arasında tek satır sonu (çift newline +1 satır kaydırıyordu). */
export const SQL_EXPORT_HEADER_BODY_SEP = '\n'

/** Yorum satırında `|` açıklama içinde geçince sütunlar kayıyor; gösterimde kırık çubuk kullan. */
function escapePipeInExportNote(s: string): string {
  return (s ?? '').replace(/\|/g, '¦')
}

/** 1-based satır numarası: full içinde abs karakter indeksinin satırı */
function lineNumberAtCharIndex(full: string, abs: number): number {
  let line = 1
  const end = Math.min(Math.max(0, abs), full.length)
  for (let i = 0; i < end; i++) {
    if (full[i] === '\n') line++
  }
  return line
...
/**
 * İndirilen .sql dosyasında bu nesnenin kaynak tanımının ilk karakterinin 1-based satırı.
 * Tam çıktı (`commentHeader + sep + body` sonra trim) üzerinde aranır; SSMS ile birebir hizalanır.
 */
export function fileLineOfFirstSourceChar(
  commentHeader: string,
  bodyText: string,
  r: ObjectReviewResult,
): number {
  const full = `${commentHeader}${SQL_EXPORT_HEADER_BODY_SEP}${bodyText}`.trim()
  const sql = (r.source_sql ?? '').trim().replace(/^\ufeff/, '')
  if (!sql) return 1
  const db = (r.database ?? '').trim()
  const exportHeader = db
    ? `-- [${db}] ${r.schema}.${r.name} (${r.object_type})`
    : `-- ${r.schema}.${r.name} (${r.object_type})`
  const firstLine = (sql.split(/\r?\n/)[0] ?? '').replace(/^\ufeff/, '')
  const needles = [`${exportHeader}\n${firstLine}`, `${exportHeader}\r\n${firstLine}`]
  /** Inceleme yorumunun disinda aranir; govde icindeki ayni metin erken eslesmesin. */
  const reviewEnd = full.indexOf('*/')
  const searchFrom = reviewEnd >= 0 ? reviewEnd + 2 : 0
  for (const needle of needles) {
    const idx = full.indexOf(needle, searchFrom)
    if (idx >= 0) {
      const abs =
        idx + exportHeader.length + (needle.includes('\r\n') ? 2 : 1)
      return lineNumberAtCharIndex(full, abs)
    }
  }
  const j = full.indexOf(firstLine, searchFrom)
  if (j >= 0) return lineNumberAtCharIndex(full, j)
  return 1
}

/** Kaynak "Satır N" → indirilen dosyada aynı kod satırına gitmek için eklenecek ofset (genelde N-1). */
function sourceLineToFileLineDelta(
  commentHeader: string,
  bodyText: string,
  r: ObjectReviewResult,
): number {
  return fileLineOfFirstSourceChar(commentHeader, bodyText, r) - 1
}

/**
 * İnceleme satır bilgisini indirilen .sql dosyası satır numaralarına çevirir
 * (Satır / Satir, tek sayı veya aralık).
 * `clampMin`: delta uygulandıktan sonra sayı alt sınırı (örn. -1 için en az 1).
 */
export function shiftLineReferenceForDownloadedSqlFile(
  loc: string,
  delta: number,
  opts?: { clampMin?: number },
): string {
  if (!loc.trim()) return loc
  if (delta === 0) return loc
  const clamp = (v: number) =>
    opts?.clampMin != null ? Math.max(opts.clampMin, v) : v
  return loc.replace(
    /\b(Satır|Satir)(\s+)(\d+)(\s*)([–-])(\s*)(\d+)\b/gi,
    (_m, w: string, sp1: string, lo: string, sp2: string, dash: string, sp3: string, hi: string) => {
      const n1 = clamp(parseInt(lo, 10) + delta)
      const n2 = clamp(parseInt(hi, 10) + delta)
      return `${w}${sp1}${n1}${sp2}${dash}${sp3}${n2}`
    },
  ).replace(
    /\b(Satır|Satir)(\s+)(\d+)\b/gi,
    (_m, w: string, sp: string, n: string) =>
      `${w}${sp}${clamp(parseInt(n, 10) + delta)}`,
  )
}

function buildSqlReviewCommentHeaderWithAdjust(
  results: ObjectReviewResult[],
  adjustLoc: (r: ObjectReviewResult, rawLoc: string) => string,
): string {
  const lines: string[] = []
  lines.push('/*')
  lines.push('SQL Review - Duzeltilmesi Gereken Yerler')
  lines.push(
    'Not: Asagidaki "Satir" degerleri bu dosyadaki mutlak satir numaralaridir (SSMS satir no ile ayni).',
  )
  let issueCount = 0
  ...
  for (const r of results) {
    ...
    for (const v of r.violations ?? []) {
      const rawLoc = (v.line_reference ?? '').trim() || 'Satir bilgisi yok'
      const loc = adjustLoc(r, rawLoc)
      const descRaw = (v.description ?? '').trim() || 'Aciklama yok'
      const desc = escapePipeInExportNote(descRaw).replace(/\s+/g, ' ')
    ...
    if (objectLines.length === 0) {
      for (const rc of r.rule_checks ?? []) {
        ...
        const rawLoc = (rc.line_reference ?? '').trim() || 'Satir bilgisi yok'
        const loc = adjustLoc(r, rawLoc)
        const descRaw = (rc.description ?? '').trim() || 'Aciklama yok'
        const desc = escapePipeInExportNote(descRaw).replace(/\s+/g, ' ')
...
/** SQL indirme: yorumdaki Satir N, dosyada nesne govdesindeki gercek satira denk gelir. */
export function buildSqlReviewCommentHeader(
  results: ObjectReviewResult[],
  bodyText: string,
): string {
  const raw = buildSqlReviewCommentHeaderWithAdjust(results, (_r, rawLoc) => rawLoc)
  return buildSqlReviewCommentHeaderWithAdjust(results, (r, rawLoc) => {
    const fileAdjusted = shiftLineReferenceForDownloadedSqlFile(
      rawLoc,
      sourceLineToFileLineDelta(raw, bodyText, r),
    )
    // SSMS ile hizalama: hesap bir satır fazla çıktığında tüm Satır N değerlerinden 1 düşür (en az 1).
    return shiftLineReferenceForDownloadedSqlFile(fileAdjusted, -1, {
      clampMin: 1,
    })
  })
}
...
export function ExportMenu({ results, falsePositives, sqlText }: Props) {
  ...
  const downloadSql = async () => {
    ...
    if (!text) return
    const commentHeader = buildSqlReviewCommentHeader(results, text)
    const sqlFile =
      `${commentHeader}${SQL_EXPORT_HEADER_BODY_SEP}${text}`.trim()
